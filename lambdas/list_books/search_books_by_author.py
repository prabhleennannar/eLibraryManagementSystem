...
def get_book_based_on_author(connection, author):
    cursor = connection.cursor()
    cursor.callproc('get_book_based_on_author', [author])
...
def handler(event, context):
    ...
    try:
        ...
        author = event['name']
        books = get_book_based_on_author(connection, author)

        if not books:
            response['status'] = 'fail'
            response['message'] = 'The books with author ' + author + ' are not available!'

        else:
            response['status'] = 'success'
            response['message'] = 'The books with author ' + author + ' are as below!'
