...
def get_book_based_on_title(connection, title):
    cursor = connection.cursor()
    cursor.callproc('get_book_based_on_title', [title])
...
def handler(event, context):
    ...
    try:
        ...
        title = event['name']
        books = get_book_based_on_title(connection, title)

        if not books:
            response['status'] = 'fail'
            response['message'] = 'The books with title ' + str(title).upper() + ' are not available!'

        else:
            response['status'] = 'success'
            response['message'] = 'The books with title ' + str(title).upper() + ' are as below!'
